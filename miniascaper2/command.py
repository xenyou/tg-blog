...
# symbols which are imported by "from miniascaper2.command import *"
...
from miniascaper2.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("miniascaper2"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="miniascaper2.config")
...
def bootstrap():
    ...
    By default this runs the function 'miniascaper2.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-miniascaper2 = miniascaper2.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="miniascaper2 %s" % version)
    ...
    _read_config(args)
    from miniascaper2.model import bootstrap_model
...
def start():
    ...
    from miniascaper2.controllers import Root
